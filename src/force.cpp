...
AirResistance::AirResistance(const Vector2 resultantForce, const float factor):
    resultantForce(resultantForce), factor(factor) {}

Vector2 AirResistance::vector() {
    if (resultantForce.x != 0) return Vector2(-factor * resultantForce.x, 0);
    if (resultantForce.y != 0) return Vector2(0, -factor * resultantForce.y);
    return Vector2(0, 0);
}
